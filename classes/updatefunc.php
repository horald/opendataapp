...
function updatesavedirect($pararray,$listarray,$filterarray,$filter,$idwert,$menu,$dbname) {
  ...
  if (isset($_REQUEST['submit'])) { 
    ...
    foreach ( $listarray as $arrelement )
    {
      if (($arrelement['type']=='blob') OR ($arrelement['type']=='pos') OR ($arrelement['type']=='textarea') OR ($arrelement['type']=='blutdruck') OR ($arrelement['type']=='average') OR ($arrelement['type']=='YN') OR ($arrelement['type']=='text') OR ($arrelement['type']=='procent') OR ($arrelement['type']=='calcsum1') OR ($arrelement['type']=='blobid') OR ($arrelement['type']=='image') OR ($arrelement['type']=='calctext') OR ($arrelement['type']=='select') OR ($arrelement['type']=='selectid') OR ($arrelement['type']=='zahl') OR ($arrelement['type']=='date') OR ($arrelement['type']=='calc')) {
