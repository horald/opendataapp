...
//  while ($line = mysql_fetch_array($result)) { 
  while ($line = db_fetch($result,$gdbtyp)) { 
    $cnt=$cnt+1;
...
//        $ressel = mysql_query($qrysel) or die(mysql_error()." qrysel:".$qrysel);
        $ressel = mysql_query($qrysel,"",$gdbtyp);
        //echo $qrysel."<br>";
      }  
    }
...
if ($iddetail=="") {
  ...
  foreach ( $filterarray as $arrelement )
  {
    ...
    $resflt = db_query($qryflt," sql001:",$gdbtyp);
    $linflt = db_fetch($resflt,$gdbtyp);
...
//        $linflt = mysql_fetch_array($resflt);
        $resflt = db_query($qryflt," sql003:",$gdbtyp);
        $linflt = db_fetch($resflt,$gdbtyp);
        $wert=$linflt[fldwert];
      }
    }  
...
//        $linsel=mysql_fetch_array($ressel);
        $ressel = db_query($qrysel," sql003b:",$gdbtyp);
        $linsel=db_fetch($ressel,$gdbtyp);
...
//      $linsel=mysql_fetch_array($ressel);
      $ressel=db_query($qrysel," sql003c:",$gdbtyp);
      $linsel=db_fetch($ressel,$gdbtyp);
      $wertid=$linsel[$filterarray[$fltcnt-1][seldbindex]];
    }    
...
//        $fresult = mysql_query($fquery) or die(mysql_error()." sql004:".$fquery);
        $fresult = db_query($fquery," sql004:",$gdbtyp);
        echo $bez." ";
...
//        while ($fline = mysql_fetch_array($fresult)) {
      ...
      break;
      case 'distinct':
        $fquery = "SELECT distinct ".$arrelement['dbfield']." FROM ".$pararray['dbtable'].$selwhere.$selorder;
        //echo $fquery."<br>";
//        $fresult = mysql_query($fquery) or die(mysql_error()." sql004a:".$fquery);
        $fresult = db_query($fquery," sql004a:",$gdbtyp);
        echo $bez." ";
        echo "<select name='".$arrelement['name']."' size='1'>";
//        while ($fline = mysql_fetch_array($fresult)) {
        while ($fline = db_fetch($fresult,$gdbtyp)) {
          $strfield = $fline[$arrelement['dbfield']];
          if ($arrelement['value'] == $strfield) {
            echo "<option style='background-color:#c0c0c0;' selected>".$strfield."</option>";
          } else {
            echo "<option style='background-color:#c0c0c0;' >".$strfield."</option>";
          }
        }
        echo "</select>";
      break;
      case 'select':
        ...
        $fresult = db_query($fquery," sql004b:",$gdbtyp);
        echo $bez." ";
        echo "<select name='".$arrelement['name']."' size='1'>";
        echo "<option style='background-color:#c0c0c0;' >(leer)</option>";
        $selectstatus=false;
//        while ($fline = mysql_fetch_array($fresult)) {
      ...
      case 'selectid':
        ...
        //$fresult = mysql_query($fquery) or die(mysql_error()." sql004c:".$fquery);
        $fresult = db_query($fquery," sql004c:",$gdbtyp);
        //$fresult=-1;
...
//        $fresult = mysql_query($fquery) or die(mysql_error()." sql005:".$fquery);
        $fresult = db_query($fquery," sql005:",$gdbtyp);
        echo $arrelement['label']." ";
        echo "<select name='".$arrelement['name']."' size='1'>";
        echo "<option style='background-color:#c0c0c0;' >(ohne)</option>";
//        while ($fline = mysql_fetch_array($fresult)) {
...
if ($drucken=="N") {
  ...
  $resfunc = db_query($qryfunc," sql006:",$gdbtyp);
  //while ($linfunc = mysql_fetch_array($resfunc)) { 
  while ($linfunc = db_fetch($resfunc,$gdbtyp)) { 
    $bez=$linfunc['fldbez'];
...
//      $resfunc = mysql_query($qryfunc) or die(mysql_error()." sql006b:".$qryfunc);
      $resfunc = db_query($qryfunc," sql006b:",$gdbtyp);
//      while ($linfunc = mysql_fetch_array($resfunc)) { 
      while ($linfunc = db_fetch($resfunc,$gdbtyp)) { 
...
//      $linmen = mysql_fetch_array($resmen); 
      $resmen = db_query($qrymen,"",$gdbtyp);
      $linmen = db_fetch($resmen,$gdbtyp); 
...
//  $linsum = mysql_fetch_array($ressum);
  $ressum = db_query($startquery," sql007:",$gdbtyp);
  $linsum = db_fetch($ressum,$gdbtyp);
...
//  $ressum = mysql_query($sumquery) or die(mysql_error()." sql007:".$sumquery);
  $ressum = db_query($sumquery," sql007:",$gdbtyp);
//  echo "</div>";
//  $linsum = mysql_fetch_array($ressum);
  $linsum = db_fetch($ressum,$gdbtyp);
...
//  $rescalc = mysql_query($calcquery) or die(mysql_error()." sql008:".$calcquery);
  $rescalc = db_query($calcquery," sql008:",$gdbtyp);
//  while ($lincalc = mysql_fetch_array($rescalc)) {
  while ($lincalc = db_fetch($rescalc,$gdbtyp)) {
...
//        mysql_query($qrykorr) or die("Error using mysql_query($sql): ".mysql_error()." sql009:".$qrykorr);
        db_query($qrykorr," sql009:",$gdbtyp);
        echo "<div class='alert alert-success'>";
...
$result = db_query($query," sql010:",$gdbtyp);
//$nr=0;
$sum=0;
$diffsum=0;
$addsum=0;
...
//        $resfch = mysql_query($qryfch) or die(mysql_error()." sql004d:".$qryfch);
//        $linfch = mysql_fetch_array($resfch);
        $resfch = db_query($qryfch," sql004d:",$gdbtyp);
        $linfch = db_fetch($resfch,$gdbtyp);
        $anz=$linfch['anz'];
        $anzsum=$anzsum+$anz;
      ...
      case 'proccalc':
        ...
        //echo $line['fldBez'].":".$qryfch."<br>";
//        $resfch = mysql_query($qryfch) or die(mysql_error()." sql004e:".$qryfch);
//        $linfch = mysql_fetch_array($resfch);
        $resfch = db_query($qryfch," sql004e:",$gdbtyp);
        $linfch = db_fetch($resfch,$gdbtyp);
        $prozwert=0;
        if ($linfch['anz']>0) {
        	 $anz2=1;
            ...
            //echo $line['fldBez'].":".$qryfch."<br>";
//            $rescnt = mysql_query($qrycnt) or die(mysql_error()." sql004f:".$qrycnt);
//            $lincnt = mysql_fetch_array($rescnt);
            $rescnt = db_query($qrycnt," sql004f:",$gdbtyp);
            $lincnt = db_fetch($rescnt,$gdbtyp);
            $anz2=$lincnt['anz'];
        	 }
      ...
      case 'procpoll':
        ...
        //echo $qryproz."<br>";        
//        $resproz = mysql_query($qryproz) or die(mysql_error()." sql004g:".$qryproz);
//        $linproz = mysql_fetch_array($resproz);
        $resproz = db_query($qryproz," sql004g:",$gdbtyp);
        $linproz = db_fetch($resproz,$gdbtyp);
      ...
      break;
      case 'combi':
        $qryort="SELECT * FROM ".$pararray['dbtable']." WHERE fldIndex=".$line['fldind_zimmer'];
        //echo $qryort."<br>";        
//        $resort = mysql_query($qryort) or die(mysql_error()." sql004h:".$qryort);
//        $linort = mysql_fetch_array($resort);
        $resort = db_query($qryort," sql004h:",$gdbtyp);
        $linort = db_fetch($resort,$gdbtyp);
        $raum=$linort['fldnummer'];
        $qryort="SELECT * FROM ".$pararray['dbtable']." WHERE fldIndex=".$line['fldind_moebel'];
        //echo $qryort."<br>";        
//        $resort = mysql_query($qryort) or die(mysql_error()." sql004i:".$qryort);
//        $linort = mysql_fetch_array($resort);
        $resort = db_query($qryort," sql004i:",$gdbtyp);
        $linort = db_fetch($resort,$gdbtyp);
        $schrank=$linort['fldnummer'];
        $wert="(".$raum."/".$schrank."/".$line['fldnummer'].")"; 
...
//        $ressel = mysql_query($qrysel) or die(mysql_error()." sql004j:".$qrysel);
//        $linsel = mysql_fetch_array($ressel);
        $ressel = db_query($qrysel," sql004j:",$gdbtyp);
        $linsel = db_fetch($ressel,$gdbtyp);
        if ($arrelement['reftable']<>"") {
        	 ...
        	 //echo $qryref."<br>";
//          $resref = mysql_query($qryref) or die(mysql_error()." sql004k:".$qryref);
//          $linref = mysql_fetch_array($resref);
          $resref = db_query($qryref," sql004k:",$gdbtyp);
          $linref = db_fetch($resref,$gdbtyp);
          echo "<td width='".$arrelement['width']."'>".$linsel[$arrelement['seldbfield']]." (".$linref[$arrelement['reffield']].")</td>";
        } else {
          echo "<td width='".$arrelement['width']."'>".$linsel[$arrelement['seldbfield']]."</td>";
        }  
      break;
      case 'image':
...
//            $fline = mysql_fetch_array($fresult);
        ...
        if ($linkfield!="") {
          ...
          if ($linkwert!="") {
            if ($line[$targetfield]!="") {
              ...
              echo "<td width='".$arrelement['width']."'><a href='".$linkwert."' target='_blank'>".$wert."</a></td>";
            }  
          } else {
            echo "<td width='".$arrelement['width']."'>".$wert."</td>";
          }  
  ...
  foreach ( $listarray as $arrelement )
  {
    ...
    {
      case 'calc':
        echo "<td style='text-align:right;padding-right:10px;' width='".$arrelement['width']."'>".sprintf("%.2f",$sum)."</td>";
      break;
      case 'cntind':
  ...
  if ($pararray['addfunc']=="true") {
    if (is_array($filterarray)) {
      ...
      case 'select':
        ...
        $fquery = "SELECT * FROM ".$arrelement['dbtable'].$selwhere;
//        $fresult = mysql_query($fquery) or die(mysql_error()." sql004l:".$fquery);
        $fresult = db_query($fquery," sql004l:",$gdbtyp);
        echo $arrelement['label']." ";
        echo "<select name='".$arrelement['name']."' size='1'>";
//        while ($fline = mysql_fetch_array($fresult)) {
        while ($fline = db_fetch($fresult,$gdbtyp)) {
